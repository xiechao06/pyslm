...
def checkStrutCylinderIntersection(
    pntA: np.array,
    pntB: np.array,
    radius: float,
    ...
    verts_3d_prev = (
        np.vstack([np.cos(theta), np.sin(theta), np.zeros(numPoints)]).T * radius
    )
    ...
    if centreOnly:
        ...
        hitLoc, index_ray, index_tri = mesh.ray.intersects_location(
            verts_3d, norm, multiple_hits=False
        )
...
def sweepPolygon(
    polygon, path, angles=None, scaleFactors=None, **kwargs
) -> trimesh.Trimesh:
    """
    ...
    base_verts_3d = tf.transform_points(base_verts_3d, tf_mat1)
...
def extrudeFace(
    extrudeMesh: trimesh.Trimesh,
    height: Optional[float] = None,
    heightArray: Optional[np.ndarray] = None,
) -> trimesh.Trimesh:
    """
    ...
    extMesh = trimesh.Trimesh(
        vertices=meshVerts, faces=meshInd, validate=True, process=True
    )
    extMesh.fix_normals()
...
def boolUnion(meshA: trimesh.Trimesh, meshB: trimesh.Trimesh) -> trimesh.Trimesh:
    ...
    outMesh = trimesh.boolean.union(
        [meshA, meshB], engine="manifold", check_volume=False
    )
    return outMesh
...
def boolIntersect(meshA: trimesh.Trimesh, meshB: trimesh.Trimesh):
    ...
    outMesh = trimesh.boolean.intersection(
        [meshA, meshB], engine="manifold", check_volume=False
    )
    return outMesh
...
def boolDiff(meshA: trimesh.Trimesh, meshB: trimesh.Trimesh) -> trimesh.Trimesh:
    ...
    outMesh = trimesh.boolean.difference(
        [meshA, meshB], engine="manifold", check_volume=False
    )
    return outMesh
...
def createPath2DfromPaths(paths: List[np.ndarray]) -> trimesh.path.Path2D:
    ...
    """
    loadedLinePaths = [
        trimesh.path.exchange.misc.lines_to_path(path) for path in list(paths)
    ]
    loadedPaths = [
        trimesh.path.Path2D(**path, process=False) for path in loadedLinePaths
    ]
...
def path2DToPathList(
    shapes: List[shapely.geometry.polygon.Polygon],
) -> List[np.ndarray]:
    """
...
def sortExteriorInteriorRings(
    polyNode, closePolygon: Optional[bool] = False
    ...
    for node in polyNode.children:
        exteriorChildRings, interiorChildRings = sortExteriorInteriorRings(
            node, closePolygon
        )
...
def triangulateShapelyPolygon(
    polygon: shapely.geometry.Polygon, triangle_args: Optional[str] = None, **kwargs
):
    """
...
def triangulatePolygonFromPaths(
    exterior: np.ndarray,
...
def triangulatePolygon(
    section, closed: Optional[bool] = False
    ...
    for polygon in section.Childs:
        ...
        # run triangulation
        triangFaces = triangulate_float32(triVerts, rings).reshape(
            (-1, 3)
...
def generatePolygonBoundingBox(bbox: np.ndarray) -> shapely.geometry.Polygon:
    ...
    bboxPoly = shapely.geometry.polygon.Polygon(
