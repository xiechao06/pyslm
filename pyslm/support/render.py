...
class Canvas(app.Canvas):
    ...
    def __init__(
        ...
        if flipDir:
            ...
            self.view = np.dot(
                np.dot(translate((-avg[0], -avg[1], -avg[2])), rotate(-180, [1, 0, 0])),
        ...
        self._rendertex = gloo.Texture2D(
            (shape + (4,)), format="rgba", internalformat="rgba32f"
        )
        ...
        self.projection = ortho(
            self.bbox[1, 0], self.bbox[0, 0], self.bbox[1, 1], self.bbox[0, 1], 2, 40
        )
    ...
    def on_draw(self, event):
        with self._fbo:
            ...
            gloo.set_viewport(0, 0, self._visSize[0], self._visSize[1])
            gloo.set_state(
                blend=True, depth_test=True, polygon_offset_fill=False, cull_face=False
            )
            ...
            self.rgb = gloo.read_pixels(
                (0, 0, self._visSize[0], self._visSize[1]), True, out_type="float"
            )
            # self.rgb = _screenshot((0, 0, *self.physical_size))


def projectHeightMap(
