...
def getContoursFromShapelyPolygon(
    poly: Polygon, mergeRings: Optional[bool] = True
...
def plotPolygon(
    polygons: List[Any],
    zPos=0.0,
    lineColor: Optional[Any] = "k",
    lineWidth: Optional[float] = 0.7,
    fillColor: Optional[Any] = "r",
    plot3D: Optional[bool] = False,
    plotFilled: Optional[bool] = False,
    handle: Tuple[plt.Figure, plt.Axes] = None,
    ...
    for contour in contourCoords:
        if plot3D:
            ...
            if plotFilled:
                ...
                ax.plot(
                    contour[:, 0], contour[:, 1], color=lineColor, linewidth=lineWidth
                )
...
def plotSequential(
    layer: Layer,
    plotArrows: Optional[bool] = False,
    plotOrderLine: Optional[bool] = False,
    ...
    for geom in layer.geometry:
        if isinstance(geom, HatchGeometry):
            ...
            coords = np.hstack([geom.coords, np.roll(geom.coords, -1, axis=0)])[
                :-1, :
            ].reshape(-1, 2, 2)
...
def plot(
    ...
    if plotHatches:
        ...
        if len(hatchGeoms) > 0:
            hatches = np.vstack(
                [hatchGeom.coords.reshape(-1, 2, 2) for hatchGeom in hatchGeoms]
            )
            ...
            if type(index) is str and str and hasattr(hatchGeoms[0], index):
                values = np.vstack(
                    [
                        np.tile(
                            getattr(hatchGeom, index),
                            [int(len(hatchGeom.coords) / 2), 1],
                        )
    ...
    if plotContours:
        for contourGeom in layer.getContourGeometry():
            ...
            if plot3D:
                ax.plot(
                    contourGeom.coords[:, 0],
                    contourGeom.coords[:, 1],
                    zs=zPos,
                    color=lineColor,
                    linewidth=lineWidth,
                )
    ...
    if plotPoints:
        ...
        if len(pointGeoms) > 0:
            scatterPoints = np.vstack(
                [pointsGeom.coords for pointsGeom in layer.getPointsGeometry()]
            )
            ...
            if plot3D:
                scatterObj = ax.scatter3D(
                    scatterPoints[:, 0], scatterPoints[:, 1], zPos, c=pntColors
                )
            else:
                scatterObj = ax.scatter(
                    scatterPoints[:, 0], scatterPoints[:, 1], c=pntColors
                )
...
def plotHeatMap(
    part: Part, z: float, exposurePoints: np.ndarray, resolution: float = 0.25
) -> Tuple[plt.Figure, plt.Axes]:
    """
    ...
    if exposurePoints.shape[1] != 3:
        raise ValueError(
            "Exposure points must include energy deposited i.e. 3rd column"
        )
