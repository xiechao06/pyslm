...
doe3 = doepy.build.space_filling_lhs(
    {"laserPower": [40, 70], "laserSpeed": [200, 500], "hatchDistance": [0.1, 0.4]},
...
x, y = np.meshgrid(
    np.linspace(buildPlateSize[0, 0], buildPlateSize[0, 1], numSamples[0]),
...
for i in range(numSamples[0]):
    for j in range(numSamples[1]):
        ...
        bstyle.laserSpeed = (
            laserParameters.laserSpeed
        )  # mm/s - Note this is used on some systems but should be set
