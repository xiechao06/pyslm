...
class HexIsland(hatching.Island):
    ...
    def localBoundary(self) -> np.ndarray:
        ...
        if HexIsland._boundary is None:
            ...
            radius = (
                self._islandWidth / np.cos(np.pi / numPoints) / 2 + self._islandOverlap
            )
        ...
        for i in np.arange(0, numIslandsX):
            for j in np.arange(0, numIslandsY):
                ...
                startX = (
                    -bboxRadius
                    + i * self._islandWidth
                    + np.mod(j, 2) * self._islandWidth / 2
                )
                startY = -bboxRadius + j * self._islandWidth * np.sin(
                    2 * np.pi / numPoints
                )
